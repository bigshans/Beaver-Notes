...
export class ProtocolManager {
  ...
  async initialize() {
    await Promise.all([
      ...
      ensureDir(join(app.getPath('userData'), 'settings')),
    ]);
  ...
  registerProtocols() {
    ...
    protocol.registerFileProtocol('settings', (request, callback) => {
      try {
        const normalizedPath = normalize(
          resolveAssetPathFromProtocolUrl(request.url, 'settings'),
        );
        if (!existsSync(normalizedPath)) {
          console.error(`Settings file not found: ${normalizedPath}`);
          return callback({ error: -6 });
        }
        ...
        console.error('Error handling settings protocol:', err);
        callback({ error: -2 });
      }
    });
  }
