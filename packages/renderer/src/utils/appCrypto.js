...
async function _cryptoDir() {
  try {
    const dataDir = await ipcRenderer.callMain('storage:get', {
      name: 'settings',
      key: 'dataDir',
      def: '',
    });
...
export async function setupAppEncryption(passphrase) {
  ...
  try {
    await ipcRenderer.callMain('fs:ensureDir', dir);
    ...
    try {
      await ipcRenderer.callMain('assetCrypto:setAppPassphrase', passphrase);
    } catch {
      // non-fatal
    }
    return { ok: true };
...
export async function verifyAppPassphrase(passphrase) {
  ...
  try {
    ...
    try {
      saltHex = await ipcRenderer.callMain(
        'fs:readFile',
        path.join(dir, 'salt')
      );
      keycheckRaw = await ipcRenderer.callMain(
        'fs:readFile',
    ...
    try {
      await ipcRenderer.callMain('assetCrypto:setAppPassphrase', pass);
    } catch {
      // non-fatal
    }
    return { ok: true };
...
export async function appFolderHasEncryption() {
  ...
  if (!dir) return false;
  try {
    return await ipcRenderer.callMain('fs:pathExists', path.join(dir, 'salt'));
  } catch {
    return false;
  }
}
...
export async function disableAppEncryption() {
  ...
  try {
    await ipcRenderer.callMain('assetCrypto:clearAppPassphrase');
  } catch {
    // ignore
  }
